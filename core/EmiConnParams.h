...
class EmiConnParams {
public:
    inline EmiConnParams(const sockaddr_storage& address_, uint16_t inboundPort_, EmiConnectionType type_) :
    address(address_),
    inboundPort(inboundPort_),
    type(type_),
    p2p() {}
    
    inline EmiConnParams(const sockaddr_storage& address_, uint16_t inboundPort_,
