...
struct EmiMessageHeader {
    ...
    EmiRttSequenceNumber rttMeasurementRequest; // Set if (flags & EMI_RTT_REQUEST_PACKET_FLAG)
    EmiRttSequenceNumber rttMeasurementResponse; // Set if (flags & EMI_RTT_RESPONSE_PACKET_FLAG)
    
    // Returns true if the parse was successful
    //
    // Note that this method does not check that the entire
    // packet fits in the buffer, only that the header fits.
    static bool parse(const uint8_t *buf, size_t bufSize, EmiPacketHeader& header);
};
